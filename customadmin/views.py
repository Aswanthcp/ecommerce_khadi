...
from orders.models import Coupons, Order, OrderItems
from store.models import Product, Variation

from .forms import (CategoryEditForm, DiscountCouponForm, ItemCreateForm,
                    OrderForm, VariationForm)
...
def admin_tab(request):
    ...
    order = OrderItems.objects.all()
    order_count = OrderItems.objects.all().count()
    context = {
...
def manage_user(request):
    if 'q' in request.GET:
        q = request.GET['q']
        if q:
            users = Account.objects.order_by(
                '-id').filter(Q(first_name__icontains=q) | Q(last_name__icontains=q) | Q(email__icontains=q))
            ...
            return redirect('customadmin:manage_user', context)
    else:
...
def block_user(request, id):
    ...
    if account.is_active:
        ...
        account.save()
    return redirect('customadmin:manage_user')
...
def edit_category(request, id):
    ...
    try:
        ...
        print("slug exists")
        return redirect('customadmin:edit_category')
...
def add_category(request):
    ...
    try:
        if request.method == 'POST':
            print('dshfhdgdfhgshdlivsdisd')
            title = request.POST['title']
            slug = request.POST['slug']
            print(title,slug)
            category = Category.objects.create(category_name=title, slug=slug)
            print(category,'=======================')
            category.save()
            return redirect('customadmin:manage_category')
...
def manage_product(request):
    if 'q' in request.GET:
        q = request.GET['q']
        if q:
            product = Product.objects.order_by('-id').filter(Q(category__title__icontains=q) | Q(
                subcategory__name__icontains=q) | Q(gender__icontains=q) | Q(brand__brand_name__icontains=q) | Q(name__icontains=q))
        ...
        if not product.exists():
            ...
            return redirect('customadmin:manage_product')
    else:
...
def edit_product(request, id):
    if request.user.is_authenticated:
        ...
        try:
            ...
            messages.error(request, "Slug already exists.")
            return redirect('customadmin:edit_product')
...
def manage_variation(request):
    if 'q' in request.GET:
        q = request.GET['q']
        if q:
            variation = Variation.objects.order_by('-id').filter(Q(product__name__icontains=q) | Q(
                variation_category__icontains=q) | Q(variation_value__icontains=q))
            # users_count = users.count()
            if not variation.exists():
                messages.error(request, 'No Matching Datas Found')
                return render(request, 'admins/variation.html')
        else:
            return redirect('customadmin:manage_variation')
    else:
    ...
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            if user.is_superadmin:
                ...
                return redirect('customadmin:admin_tab')
    ...
    if 'q' in request.GET:
        ...
        if q:
            order = Order.objects.order_by(
                '-id').filter(Q(user__first_name__icontains=q) | Q(order_number__icontains=q))
            order_product = OrderItems.objects.all()
            # users_count = users.count()
            if not order.exists():
...
def manage_discount(request):
    if 'q' in request.GET:
        q = request.GET['q']
        if q:
            coupon = Coupons.objects.order_by(
                '-id').filter(Q(coupon_code__icontains=q))
            # users_count = users.count()
            if not coupon.exists():
                messages.error(request, 'No Matching Datas Found')
                return render(request, 'admins/coupon.html')
        else:
            return redirect('customadmin:manage_discount')
    else:
...
def sales_report(request):
    ...
    yearly = OrderItems.objects.filter(
        created_at__lte=ymax, created_at__gte=ymin)
    mmax = timezone.now()
    mmin = (timezone.now() - datetime.timedelta(days=30))
    monthly = OrderItems.objects.filter(
        created_at__lte=mmax, created_at__gte=mmin)
    ymax = timezone.now()
    ymin = (timezone.now() - datetime.timedelta(days=7))
    weekly = OrderItems.objects.filter(
        created_at__lte=ymax, created_at__gte=ymin)
    month_sale = ["", "", "", "", "", "", "", ""]
    subm = timezone.now()
